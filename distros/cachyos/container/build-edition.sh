...
insert = (
    ...
    '        # The boost NEEDED entry is in libcalamares.so, not the calamares binary itself.\n'
    '        _calamares_lib=$(find "${pacstrap_dir}/usr/lib" -name "libcalamares.so*" -type f 2>/dev/null | head -1)\n'
    '        if [[ -n "${_calamares_lib}" ]]; then\n'
    '          _boost_needed=$(LC_ALL=C readelf -d "${_calamares_lib}" 2>/dev/null | grep -oP "libboost_python314.so.[0-9.]+" | head -1)\n'
    '          _boost_present=$(find "${pacstrap_dir}/usr/lib" -name "${_boost_needed}" 2>/dev/null | head -1)\n'
    '          if [[ -n "${_boost_needed}" && -z "${_boost_present}" ]]; then\n'
    '            _boost_ver=$(echo "${_boost_needed}" | grep -oP "[0-9]+\\.[0-9]+\\.[0-9]+" | head -1)\n'
    '            printf "parental-os: calamares needs %s but it is missing; downgrading boost-libs to %s\\n" "${_boost_needed}" "${_boost_ver}"\n'
    '            _boost_pkg="boost-libs-${_boost_ver}-1-x86_64.pkg.tar.zst"\n'
    '            _boost_url="https://archive.archlinux.org/packages/b/boost-libs/${_boost_pkg}"\n'
    '            _boost_tmpfile="$(mktemp -p /tmp boost-libs.XXXXXX.pkg.tar.zst)"\n'
    '            if curl -fsSL "${_boost_url}" -o "${_boost_tmpfile}"; then\n'
    '              if ! pacman --root "${pacstrap_dir}" --noconfirm -U "${_boost_tmpfile}"; then\n'
    '                printf "parental-os: WARN: boost-libs downgrade failed; calamares may crash on startup\\n" >&2\n'
    '              fi\n'
    '            else\n'
    '              printf "parental-os: WARN: failed to download %s; calamares may crash on startup\\n" "${_boost_pkg}" >&2\n'
    '            fi\n'
    '            rm -f "${_boost_tmpfile}"\n'
    '          fi\n'
    '        fi\n'
